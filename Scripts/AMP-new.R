...
# File paths
INPUT_FILE <- file.path("Data", CANCER_TYPE, paste0("2024_", CANCER_TYPE, "_data.csv"))
OUTPUT_FILE <- file.path("Data", paste0("lifetime_risk_2024_2022_", CANCER_TYPE, ".csv"))
...
calculate_amp_lifetime_risk <- function(region_data, distribution = "Poisson") {
  ...
  region_data <- region_data %>%
    ...
    mutate(
      # Age interval width
      wi = case_when(
        AgeEnd == 999 ~ 5,  # Assume 15 years for 85+ group (could be adjusted)
